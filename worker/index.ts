    ...
    if (path === '/auth/verify' && request.method === 'GET') {
      ...
      if (!otp) {
        ...
        return new Response(null, {
          status: 302,
          headers: { Location: `/login?error=link_expired&email=${encodeURIComponent(normalizedEmail)}` },
        });
      }
      ...
      if (!user) {
        return new Response(null, { status: 302, headers: { Location: `/login?error=not_found&email=${encodeURIComponent(normalizedEmail)}` } });
      }
